...
SELECT 
	a.WrkItmID,  
	case 
	when d.CorporateName is null or d.CorporateName in ('0', 'NULL')   then (rtrim(ltrim(FName)) +' '+ rtrim(ltrim(MName)) +' '+ rtrim(ltrim(SName))) 
...
d.ProductType,
    ...
    CASE 
        ...
        WHEN a.workStepName IN ('Checker', 'Appraisal', 'Appraisal Manager') 
             AND a.Decision IN ('Clarification', 'Forward','Exception Raised', 'Refer', '--Select--') THEN 'IN-HOUSE ACTIONS'
        WHEN a.workStepName IN ('Checker', 'Appraisal', 'Appraisal Manager') 
